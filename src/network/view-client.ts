...
import { RemoteCacheService } from './remote-cache'
...
function createViewClient(
  ...
  async function loadFromCache<Schema>(url: string, method: HttpMethod = 'get') {
  ...
  async function getUITreeCacheProtocol<Schema>(
    response: Response,
    url: string,
    method: HttpMethod,
  ...
  async function loadFromServer<Schema>(
    url: string,
  ...
  async function load<Schema>({
    ...
    async function loadNetwork(hasPreviousSuccess = false, useBeagleCacheProtocol = true) {
      ...
      const defaultHeaders = await remoteCache.getHeaders(url, method)
      const requestHeaders = { ...headers, ...defaultHeaders }
      onChangeTree(await loadFromServer(url, method, requestHeaders, strategy !== 'network-only', useBeagleCacheProtocol))
    }
