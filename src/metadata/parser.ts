...
import { BeagleConfig, Lifecycle } from 'service/beagle-service/types'
import { ComponentMetadata, ExtractedMetadata } from 'metadata/types'
...
function extract(components: BeagleConfig<any>['components']) {
  ...
  keys.forEach((key) => {
    ...
    if (!metadata) return
    if (metadata.children) extractedMetadata.children[key.toLowerCase()] = metadata.children
